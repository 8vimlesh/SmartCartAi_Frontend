  ...
  return (
    <div className="cards" style={{ display: "grid" }}>
      {avail.map(([k, d], i) => {
        ...
        const bw = d.p
          ? Math.round(((d.p - minP) / (maxP - minP || 1)) * 60 + 40)
          : 0;
        const animDelay = `${i * 0.06}s`;

        const ratingValue = Number.isFinite(d.r)
          ? Math.min(5, Math.max(0, Number(d.r)))
          : 0;
        ...
        return (
          ...
          >
            ...
            <div className="pcard-img-area">
              {data.img ? (
                <img
                  className="pcard-prod-img"
                  src={data.img}
                  alt={`${data.name} on ${pfc.n}`}
                />
              ) : (
            ...
            <div className="pcard-body">
              <div className="pcard-row1">
                <div className="pcard-logo" style={{ background: pfc.s }}>
                  {pfc.em}
                </div>
                <div
                  className="pcard-store"
                  style={{ color: isBest ? pfc.c : undefined }}
                >
                  {pfc.n}
                </div>
                <div className="pcard-revs">
                  {(d.rv || 0).toLocaleString("en-IN")} reviews
                </div>
              </div>
              ...
              <div>
                <div
                  ...
                  style={{ color: isBest ? pfc.c : "var(--ink)" }}
                >
                ...
                <div className="pcard-price-sub">
                  ...
                  <span
                    className="pcard-off"
                    style={{ background: pfc.s, color: pfc.c }}
                  >
                    {d.disc || 0}% off
                  </span>
              ...
              <div className="pcard-tags">
                {d.ship ? (
                  <span className="ptag fs">✓ Free ship</span>
                ) : (
                ...
                {d.of && d.of[0] ? (
                  <span className="ptag off_">🏷 {d.of[0]}</span>
                ) : null}
              </div>
              {d.emi && (
                <div
