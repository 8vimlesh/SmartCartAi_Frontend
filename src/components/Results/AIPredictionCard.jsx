...
export default function AIPredictionCard({ data }) {
    ...
    const fetchPrediction = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${baseUrl}/api/predict?product=${encodeURIComponent(data.name)}`,
        );
        ...
        try {
          ...
          throw new Error(
            `Server returned non-JSON response: ${text.substring(0, 30)}...`,
          );
        }
              ...
              >
                <div
                  style={{
                    ...
                    marginBottom: "12px",
                    background:
                      "linear-gradient(90deg, var(--off2), var(--off3), var(--off2))",
  ...
  const pfc = PF[insight.best_platform] || {
    n: insight.best_platform,
    c: "var(--ink)",
  };
  ...
  return (
    ...
    >
      ...
      >
        <span className="card-hd-title" style={{ color: "var(--ink)" }}>
          🧠 Cortex AI Insights
        </span>
        <span className="card-hd-sub" style={{ color: "var(--ink3)" }}>
      ...
      <div className="card-body" style={{ padding: "24px" }}>
        <div style={{ display: "flex", gap: "20px", flexWrap: "wrap" }}>
          ...
          >
            ...
            >
              Current:{" "}
              <strong>₹{insight.current_avg?.toLocaleString("en-IN")}</strong>{" "}
              <br />
              Predicted:{" "}
              <strong>
                ₹{insight.predicted_price?.toLocaleString("en-IN")}
              </strong>
              <span
                style={{
                  marginLeft: "8px",
        ...
        >
          *AI predictions are generated using 30 days of historical data and
          simulated noise modeling.
        </div>
      </div>
    </div>
  );
