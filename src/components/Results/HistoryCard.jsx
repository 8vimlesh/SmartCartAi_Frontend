        ...
        <table className="htbl">
          <thead>
            <tr>
              ...
              {Array.from({ length: Math.max(0, 3 - topKeys.length) }).map(
                (_, i) => (
                  <th key={`empty-${i}`}>—</th>
                ),
              )}
            ...
            {rec.map((h, i) => {
              ...
              return (
                <tr key={i}>
                  ...
                  <td
                    className="ht-p"
                    style={{ color: isLow ? "var(--green)" : undefined }}
                  >
                    ₹
                    {typeof h?.p === "number"
                  ...
                  {topKeys.slice(1).map((k) => {
                    ...
                    if (!hk)
                      return (
                        <td
                          key={k}
                          style={{ color: "var(--ink4)", fontSize: "12px" }}
                        >
                          —
                        </td>
                      );
                    ...
                    return (
                      <td key={k} className="ht-p">
                        {m ? `₹${m.p.toLocaleString("en-IN")}` : "—"}
                      </td>
                    );
                  })}
                  {Array.from({
                    length: Math.max(0, 2 - topKeys.slice(1).length),
                  }).map((_, idx) => (
                    <td
                      key={`pad-${idx}`}
                      style={{ color: "var(--ink4)", fontSize: "12px" }}
