      ...
      <div className="card-body">
        <div className="al-grid">
          {avail.map(([k, d]) => {
            ...
            return (
              <div
                key={k}
                className={`al-row ${fired ? "fired" : watch ? "watching" : ""}`}
              >
                <div
                  className="al-logo"
                  style={{ background: pfc.s, border: `1.5px solid ${pfc.b}` }}
                >
                  {pfc.em}
                </div>
                <div className="al-nm">
        ...
        <div className="notifs">
          {notifs.map((n, i) => {
            ...
            const currentPrice =
              typeof n.p === "number" ? n.p.toLocaleString("en-IN") : "—";
            const thresholdPrice =
              typeof n.thr === "number" ? n.thr.toLocaleString("en-IN") : "—";
            const diffPrice =
              typeof n.p === "number" && typeof n.thr === "number"
                ? (n.thr - n.p).toLocaleString("en-IN")
                : "—";
            if (n.t === "fired") {
              return (
                <div key={i} className="notif fired">
                  ...
                  — ₹{currentPrice} is below target ₹{thresholdPrice}. Save ₹
                  {diffPrice}!
                </div>
              );
            }
