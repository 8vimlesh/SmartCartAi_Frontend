...
export default function Loading({ query, activePF, searchData }) {
  ...
  const foundKeys = searchData?.pf
    ? Object.keys(searchData.pf).filter((k) => searchData.pf?.[k]?.p)
    : [];

  const doneCount = foundKeys.length;
  const totalCount = keys.length;
  const progressPct = searchData
    ? 100
    : Math.min(90, Math.round((doneCount / Math.max(totalCount, 1)) * 80) + 10);
  ...
  return (
    <div className="loading" style={{ display: "block" }}>
      ...
      <div className="loading-sub">
        {searchData
          ? "Rendering results…"
          : "Scanning live prices across platforms"}
      </div>
      ...
      <div className="prog-lbl">
        {searchData
          ? "Complete ✓"
          : `Scanning ${doneCount}/${totalCount} platforms…`}
      </div>
    </div>
  );
}
